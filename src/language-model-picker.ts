...
function getModelGroupLabel(model: vscode.LanguageModelChat): string {
  const providerName = getExtensionProviderName(model);
  if (providerName) {
    const displayName = getProviderPickerDisplayName(providerName);
    if (isUnifyChatProviderVendor(model.vendor) || model.vendor === displayName) {
      return displayName;
    }
  }
  return getModelVendorLabel(model);
}
...
function createModelQuickPickItem(
  ...
  return {
    label: model.name,
    description: getModelVendorLabel(model),
    detail: model.id,
    model,
    resolvedModel: model,
  };
...
export async function pickLanguageModel(
  ...
  const groupedItems: LanguageModelQuickPickItem[] = [];
  if (options.includeDefault) {
    groupedItems.push(createDefaultModelQuickPickItem());
  }

  let lastGroupLabel: string | undefined;
  for (const item of orderedModelItems) {
    const groupLabel = item.resolvedModel
      ? getModelGroupLabel(item.resolvedModel)
      : item.model.vendor;
